...
const timelineItems = [
  {
    ...
    title: 'SSAFY 전공자 Java 트랙 수료',
    company: '삼성 청년 SW·AI 아카데미',
    description:
      '프로젝트 우수상 2회 수상, AI 기반 프로젝트 개발 SW 역량테스트 모의 A형 통과, 2월 이달의 동료상',
    important: true,
  ...
  },
  {
    ...
    company: '㈜가온아이',
    description:
      'Vue.js 기반 그룹웨어 솔루션 개발, 다국어 지원 구현, 조직도/그룹/쪽지/메신저 기능 개발, CI/CD 및 유지보수 담당',
    important: true,
  ...
  },
  {
    ...
    description:
      '특모범상 수상, Spring 기반 웹 개발 역량 확보, Java 개발자 양성 과정 이수',
    important: false,
  ...
  },
  {
    ...
    company: '홍익대학교',
    description:
      '경영학 전공, IT 서비스 기획 및 개발 역량 융합, 서울시 마포구 본교 소재',
    important: true,
  ...
  return (
    <section
      ...
      <motion.div
        ...
        <div className="text-lg text-subText  font-semibold">
          실무형 프론트엔드 개발자
        </div>
      </motion.div>
          ...
          {timelineItems.map((item, index) => (
            <motion.div
              ...
              <div
                className={`${
                  item.important
                    ? 'bg-surface  rounded-2xl p-6 shadow-custom border border-border  hover:shadow-lg'
                    : 'bg-transparent p-4'
                } transition-shadow`}>
                <div className="flex items-center gap-4 mb-2">
                  <span
                    className={`${
                      item.important ? 'text-2xl' : 'text-lg'
                    } font-bold text-primary `}>
                    {item.year}
                  </span>
                  <h3
                    className={`${
                      item.important ? 'text-xl' : 'text-base'
                    } font-bold text-text `}>
                    {item.title}
                  </h3>
                </div>
                <div
                  className={`${
                    item.important
                      ? 'text-subText  font-semibold'
                      : 'text-subText/80  font-medium'
                  }  mb-2`}>
                  {item.company}
                </div>
                <p
                  className={`${
                    item.important ? 'text-text ' : 'text-text/80 '
                  } ${item.important ? '' : 'text-sm'}`}>
                  {item.description}
                </p>
              </div>
            </motion.div>
